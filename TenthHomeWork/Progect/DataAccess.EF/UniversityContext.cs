...
namespace DataAccess.EF
{
    public class UniversityContext: DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            //          LECTURER
            modelBuilder.Entity<LecturerCourse>().HasKey(p => new { p.CourseId, p.LecturerId });
            modelBuilder.Entity<Lecturer>().HasMany<LecturerCourse>().WithOne(p => p.Lecturer).HasForeignKey(p => p.LecturerId);
            modelBuilder.Entity<Lecturer>().HasMany<LecturerCourse>().WithOne(p => p.Lecturer).OnDelete(DeleteBehavior.Cascade);

            modelBuilder.Entity<StudentCourse>().HasKey(k => new { k.CourseId, k.StudentId });
            modelBuilder.Entity<Student>().HasMany<StudentCourse>().WithOne(p => p.Student).OnDelete(DeleteBehavior.Cascade);
            modelBuilder.Entity<Student>().HasMany<StudentCourse>().WithOne(p => p.Student).HasForeignKey(p => p.StudentId);

            modelBuilder.Entity<Course>().HasMany<StudentCourse>().WithOne(p => p.Course).HasForeignKey(p => p.CourseId);
            modelBuilder.Entity<Course>().HasMany<StudentCourse>().WithOne(p => p.Course).OnDelete(DeleteBehavior.Cascade);

            modelBuilder.Entity<Course>().HasMany<LecturerCourse>().WithOne(p => p.Course).HasForeignKey(p => p.CourseId);
            modelBuilder.Entity<Course>().HasMany<LecturerCourse>().WithOne(p => p.Course).OnDelete(DeleteBehavior.Cascade);
